...
export function ExpenseItem({ id, category, item, amount, participantName, onEdit, onDelete, delay = 0 }: ExpenseItemProps) {
  ...
  return (
    ...
    >
      <div className="text-2xl select-none">{CATEGORY_ICONS[category] || '📦'}</div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
          ...
          <span>{t("paid.by")} <span className="text-foreground/80 font-medium">{participantName}</span></span>
        </p>
      </div>

      <div className="text-right">
        <div className="text-lg font-bold text-gold tabular-nums">${Number(amount).toFixed(2)}</div>
      </div>

      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-blue hover:bg-blue/10" onClick={onEdit}>
