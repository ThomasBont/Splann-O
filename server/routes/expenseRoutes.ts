...
router.put(p(api.expenses.update.path), requireAuth, asyncHandler(async (req, res) => {
  ...
  const input = bodySchema.parse(req.body);
  const txResult = await expenseService.updateExpenseForPlan({
    expenseId,
  ...
  if (changed) {
    ...
    const bbq = await getBarbecueOr404(req, updated.barbecueId);
    const currency = bbq.currency ?? "€";
    ...
    await postSystemChatMessage(updated.barbecueId, `${actorName} updated ${updated.item}`, {
      ...
      amount: Number.isFinite(amount) ? amount : 0,
      currency,
      paidBy: txResult.payerParticipant.name || actorName,
    });
  }
...
router.delete(p(api.expenses.delete.path), requireAuth, asyncHandler(async (req, res) => {
  ...
  const txResult = await expenseService.deleteExpenseForPlan({
    expenseId,
    userId: sessionUserId,
    username,
  });
  ...
  if (txResult.activityRow) {
    broadcastPlanActivityCreated(txResult.expense.barbecueId, txResult.activityRow);
    broadcastEventRealtime(txResult.expense.barbecueId, {
      type: "expense_deleted",
      eventId: txResult.expense.barbecueId,
      expenseId,
    });
  }

  await postSystemChatMessage(txResult.expense.barbecueId, `${username} deleted ${txResult.expenseTitle}`, {
    type: "expense",
    action: "deleted",
    expenseId,
    item: txResult.expenseTitle,
    amount: Number.isFinite(txResult.amount) ? txResult.amount : 0,
    currency: txResult.currency,
    paidBy: txResult.paidByName,
  });
  await broadcastPlanBalancesUpdated(txResult.expense.barbecueId);
  res.status(204).send();
}));
